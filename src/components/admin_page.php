...
$username =  $_SESSION['username']; //getting log in status
...
if (isset($_GET ["action"]) && $_GET ["action"] === "logout"){ //admin logout
    ...
    header("Location:admin_loginpage.php"); //go to the location after destroying
    exit;
}
if (isset($loggedIn)){ //if admin had logged in
    ...
    $logout = '<a href="admin_page.php?action=logout">Logout</a>'; //variables for footer links
    $register = '<a class="account" href="register.php">Registreren</a>';
} else {
    header('Location:admin_loginpage.php'); //if not logged in, go to the location
    exit;
}
...
$result = mysqli_query($db, $query);
header('Location:admin_page.php'); //go to location after deleting
exit; //exit header
}
...
if(isset($idAdd)){ //if add has been used
    ...
    header ('Location:admin_page.php'); //go to location after adding
    exit(); //exit header
}
...
<div class="panel panel-default">
    ...
    <table class="table">
        ...
        <div class="panel panel-default">
            ...
            <form action="admin_page.php" id="main-contact-form" class="contact-form" name="contact-form" method="post">
            <table class="table">
                <thead>
                <tr>
                    <th >#</th>
